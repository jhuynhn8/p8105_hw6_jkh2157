...
- 2.5th percentile and 97.5th percentile of the bootstrap distribution serve as the lower and upper bounds of the 95% CI.
- The 95% CI for R^2 (0.934–0.947) is very narrow, showing that the model’s explanatory power is strong and highly stable.
- The much wider CI for the coefficient ratio (–275 to –125) reflects high variability, driven by the very small precipitation coefficient.
...
**Explanation**: I created a linear model for bwt that included gestational age, infant sex, maternal age, maternal pre pregnancy weight, weight gain, and smoking, since these variables are known to be well-established predictors of fetal growth. I then used `add_predictions()` and `add_residuals()` to look at the adequacy of the model. The plot was dense but mostly centered with only mild curvature at the ends, which means that it should be a reasonable model for these predictors. 
...
tidy(model_ga) |>
  kable(digits = 3, caption = "Gestational Age Only Model")

model_interaction =
  lm(bwt ~ bhead * blength * babysex, data = birthweight_df)

tidy(model_interaction) |>
  kable(digits = 3, caption = "Three-Way Interaction Model (Head × Length × Sex)")
```
...
cv_df =
  ...
  mutate(
    ...
    fit_main   = map(train, ~ lm(bwt ~ gaweeks + babysex + momage + ppwt + wtgain + smoken, data = .x)),
    ...
    rmse_ga    = map2_dbl(fit_ga,    test, ~ sqrt(mean((predict(.x, .y) - .y$bwt)^2))),
    rmse_inter = map2_dbl(fit_inter, test, ~ sqrt(mean((predict(.x, .y) - .y$bwt)^2)))
  )
...
cv_df |>
  summarize(
    ...
    mean_rmse_ga    = mean(rmse_ga),
    mean_rmse_inter = mean(rmse_inter)
  ) |>
  kable(digits = 2, caption = "Cross-Validated RMSE Comparison")

# curious to see what it looks like on plot 

cv_df |>
  select(rmse_main, rmse_ga, rmse_inter) |>
  pivot_longer(everything(), names_to = "model", values_to = "rmse") |>
  ggplot(aes(x = model, y = rmse)) +
  geom_boxplot(fill = "coral", alpha = 0.6) +
  labs(
    title = "Cross-Validated RMSE for All Models",
    x = "Model",
    y = "RMSE"
  )
...
- Main: this was my model 
- ga: this was the model with blegnth and gaweeks 
- inter: this was the interaction model with bhead * blength * babysex
- After these analyses, the cross-validated RMSE showed that the theory-driven model had the highest prediction error, while the model using length and gestational age performed noticeably better. The full interaction model using head circumference, length, and sex produced the lowest RMSE, indicating the strongest predictive performance.
